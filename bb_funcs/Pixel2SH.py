"""
Functions necesary to convert a segmented and a target pixel value
to a table of Spherical Harmonics coefficients and cartessian coords
The final objective of the package is to have 

Pixel2SH(segmented_im, pixelvalue, pxy, pz, ExpansionDegree) --> coords, SHTable

"""
import pyshtool as sh
import scipy 
import pyclesperanto_prototype as cle
from .Tools import sphereFit, cart2sph, Rotate, SH2NP
import numpy as np



def EdgesFromPixelValue(segmented_im, pixelvalue=1, crop_buffer=1):
    '''
    Returns a binary image of the bead with given
    pixel value from segmented image
    '''
    masked_im = (segmented_im==pixelvalue)*1
    surface = cle.detect_label_edges(masked_im)
    im_binary = cle.pull(surface).astype(bool)
    return im_binary


def FullC20_optimizer(im_binary, px=1, pz=1, ExpDegree=5):
    """
    Given a binary image and the pixel sizes, extract coords
    and optimizes a rotation to create a minimal c_20 value
    Returns 
    
    """
    # Extract coords
    y = np.where(im_binary==1)[0] * pz
    z = -np.where(im_binary==1)[1] * px
    x = np.where(im_binary==1)[2] * px
    coords = (x,y,z)
    
    # Run optimization and finds optimal rotation
    ...
    optimal_rot_x, optimal_rot_y = output[0]
    
    # Performs final rotation, creates outputs
...
def C20_rotation_outputs(angles, TIFF, ExpDegree, Voxel_XY, Voxel_Z):
    ...
    in order to get all the necessary outputs. This can be refactored to do everything at once
    '''
