...
def create_tables_from_table(table, units='um'):
    '''
    Creates a table of Spherical Harmonics coefficients, both in real and complex
    form (for calculations and plots purposes respectively) from an EXISTING sh.Coeffs table (in memory)
    Modified from create_table for StreamLit
    Returns:
        lmax: highest order of expansion available
        coeff_table_real: real coeffs
        coeff_table_complex: complex coeffs
...
def ParameterSubstitution(expr, coeff_table, initial_radius, G_exp, nu_exp, Order, first_n=0): 
    ...
    print(f'Substitution of parameters took {int(time()-start)} seconds')
    return expr.evalf()
...
def ParameterSubstitutionReplace(expr, coeff_table, initial_radius, G_exp, nu_exp, Order, first_n=0): 
    ...
    substs[G] = G_exp
    substs[nu] = nu_exp
    print(f'\nGeneration of coefficient dictionary took {np.round(time()-start, 6)} seconds')
    start = time()
    expr = expr.xreplace(substs)
    print(f'Replacement of parameters took {np.round(time()-start, 6)} seconds')
    return expr.evalf(5)
...
def BeadSolverFromTable(table, order=5, G_exp=1, nu_exp=0.45, N_lats=50, N_lons=100):
    '''
    The main function to use to solve the stress tensor associated of a bead which
    has been expanded into a table os Spherical Harmonics.
    '''
